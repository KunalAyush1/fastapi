...
def find_index_post(id):
    for i, p in enumerate(my_posts):
        if p['id'] == id:
            return i #returns index of that specific id
...
@app.post("/posts", status_code = status.HTTP_201_CREATED)#if we want to throw a specific HTTP status code , pass it as parameter in the decorator
